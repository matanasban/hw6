...
void magazine_count_appear(char** magazine ,int magazine_count,int* words_count);
void note_aount_appear(char** note ,int note_count,int* words_count);
...
//Checks if the number of appears of a certain word is the same in both lists.
bool check_data(struct node* head,char* key_to_cmp,int data_to_cmp){
    ...
    while (current != NULL){
        ...
        if (!(strcmp(key_to_cmp,temp_word))){//found the key
            ...
            if (temp_data < data_to_cmp){//number of appearences is lower in magazine
                return false;
            }
        }
...
void free_list(struct node* head){
    ...
    //Creates an array with with the appearnces of each word in magazine
void magazine_count_appear(char** magazine ,int magazine_count,int* words_count){
       int count = 0,j,i;
...
void magazine_map(struct node* temp_magazine,char** magazine,int n){
    ...
    for(i=1;i<n;i++){
         if(in_list(head_1,magazine[i])){//Key already exist , skip adding it to the list.
            continue;
        } else {
        struct node* temp_magazine2= malloc(sizeof(struct node));//Add the key to the list
...
void note_map(struct node* temp_note,char** note,int n){
    ...
    head_2 = temp_note;
    for(i=1;i<n;i++){
        if(in_list(head_2,note[i]))//Key already exist , skip adding it to the list.
            continue;
        else{
        struct node* temp_note2 = malloc(sizeof(struct node));//add new key to the list.
        if(temp_note2 == NULL){
           	fprintf(stderr,"Failed to Allocate Bytes");
   	 		abort();
   	 	}
...
// Checks if the note contained in the magazine
bool Contained(){
